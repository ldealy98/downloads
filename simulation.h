...
typedef struct checkPtSpace_struct checkPtSpace;
...
// Record the mean and max temperatures over all time and space (in the float 
// values pointed to by mean and max, each just a single float passed by reference).
// runSim does do the initialization of the checkPtTime
// and checkPtSpace structs automatically at the beginning of the simulation.
