...
// every stepsPerCheckPt time steps (none if stepsPerCheckPt=0). Record the time
// series of temperature values at row yID and column xID in the checkPtSpace.
// // Record the mean and max (evaluated over all space and time in this simulation,
// each just a single float passed by reference). runSim does do the initialization 
// of the checkPtTime and checkPtSpace struct automatically at the beginning of the simulation.
// Note: running the simulation doesn't also initialize the sim or the material. Do them separately.
int runSim(sim *thisSim, int nSteps, int stepsPerCheckPt, checkPtTime *theseTimes, float *mean, float *max, unsigned int xID, unsigned int yID, checkPtSpace *thisPt){
	int flag = 0; // return flag, 0 if no problem, but nonzero if there's a problem

	// do the initialization of the checkpointing struct for snapshots of whole temp field in time
	int nSnaps = 0;
	if(stepsPerCheckPt > 0){
		nSnaps = calcNSnaps(nSteps,stepsPerCheckPt);
	...
	// ================YOUR CODE HERE================================
	// do the initialization of the checkpointing struct for time series at xID, yID
	// ===============END OF YOUR CODE================================


	// run through the steps
	float maxVal = 0; // will overwrite with max value across array over time
	double meanVal = 0; // will add onto with all array temperatures
	int step;
	if(nSnaps > 0) recordSnap(theseTimes); // always record 0th  time step if whole-material snapshots are recorded
	flag += recordStateHist(thisPt); // always record 0th time step at point of interest

    // =================YOUR CODE HERE==========================
    // compare all points to max value and update

    // add all points to mean value
        
    // ==============END OF YOUR CODE===========================

	// actually start iterating through time steps
	for(step=1; step<nSteps; ++step){
	    
		int stepFlag = oneStep(thisSim); // actually update your arrays with next time step
		if(stepFlag){
			printf("WARNING: issue in simulation at %d time step \n",step);
			flag += stepFlag;
		}

		if((nSnaps > 0) && (step % stepsPerCheckPt == 0)) recordSnap(theseTimes); // this checks if a checkpoint needs to be recorded in whole region and records it if needed
		
		// =================YOUR CODE HERE ====================
		// in code block below record temperature at point of interest
		// calc number of points in material
		// for each point in the material
		    // grab temperature at this location
		    // compare each value to current max value (replace max value if bigger)
		    // add each temperature to the overall array sum
		
		// ===============END OF YOUR CODE ====================

	}
	// adjust mean value by number of spatial * time points
	*mean = (float)meanVal / (float)((thisSim->thisMaterial)->Nx *  (thisSim->thisMaterial)->Ny * nSteps);
